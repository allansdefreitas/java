...
function registerEvents() {
    ...
    menu.onmouseout = function () { //onmouseout event occurs when the mouse pointer is moved out of an element
        var submenu = document.getElementById('submenu');
        submenu.style.visibility = 'hidden';
    };
    var td3 = document.getElementById("td3");
    var submenu = document.getElementById("submenu");
    td3.onmouseover = submenu.onmouseover = function () {
...
function getData() {
    ...
    if (xHRObject.readyState == 4 && xHRObject.status == 200) {
        ...
        if (window.ActiveXObject) {
            ...
            var menuhere = document.getElementById("menuhere");
            if (menuhere != null) {
                menuhere.innerHTML = transform;
            }
        } else {
